...
// Lean on-chain coverage for the remaining veridoc paths not yet exercised by
// the first e2e run: update_subject, set_allowed_domains, withdraw_stake,
// verify -> VERIFIED, owner reverify, challenge (self/unstaked/real).
// Uses 30s polling to stay far under the 500 req/hour studionet budget.

const LOG = path.join(__dirname, "e2e3.log");
function log(msg) {
...
async function waitFinal(client, tx, label) {
  return client.waitForTransactionReceipt({
    hash: tx, status: "FINALIZED", fullTransaction: true, interval: 30000, retries: 300,
  });
}
...
async function read(client, fn, args, label = fn) {
  try {
    ...
    log(`  view ${label} FAIL: ${(e.message || "").slice(0, 150)}`);
    return null;
  }
}

async function main() {
  fs.writeFileSync(LOG, `=== veridoc e2e3 ${new Date().toISOString()} ===\n`);
  ...
  const SUBJECT = "lean" + stamp;
  // example.com page literally contains "illustrative examples" -> likely VERIFIED.
  ...
  // update_subject (owner) success
  await submit(client, owner, "update_subject", [SUBJECT, "Example Site Ltd", "Updated description", "organization", "low"], "update_subject");
  // set_allowed_domains (owner) success
  await submit(client, owner, "set_allowed_domains", [SUBJECT, "example.com"], "set_allowed_domains");
  await read(client, "get_subject", [SUBJECT], "subject after updates");
  ...
  if (status === "VERIFIED") {
    ...
    log("  INITIAL VERIFY NOT VERIFIED -> challenge/reverify-succ skipped (outcome above).");
  }
